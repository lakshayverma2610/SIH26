...
from ml_inference.core.schemas import ExtractedFeatures, InferenceResult

logger = get_logger(__name__)

class FraudScorer:
    """
    High-throughput ML inference engine supporting:
    1. Treelite GTIL (Ultra-low microsecond tree inference)
    2. Deterministic Smurfing / Dormancy-Break heuristic overrides
    """
    ...
    def _load_model(self) -> None:
        ...
        if treelite and self.treelite_path.exists():
            try:
                ...
                logger.warning(f"Treelite loading failed ({e}). Falling back to Heuristics.")
