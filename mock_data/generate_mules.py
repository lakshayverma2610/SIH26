...
Outputs to mock_data/data/accounts.json and mock_data/data/chains.json
"""
import sys
import json
import random
...
def generate_mules(num_chains: int = 250, num_baseline_accounts: int = 5000):
    ...
    atms_file = Path(__file__).resolve().parent / "data" / "atms.json"
    atms = []
    if atms_file.exists():
    ...
    # Cybercrime categories as defined in I4C NCRP
    crime_categories = [
        "APK_RAT_FRAUD",
        "AEPS_BIOMETRIC_FRAUD",
    ...
    c2_ips = [
        "185.220.101.4", "194.26.29.112", "45.154.255.88", "193.109.85.14",
        "103.152.18.22", "185.246.221.79"
    ]

    telegram_channels = [
        "t.me/jamtara_instant_payouts", "t.me/mewat_fast_cash", "t.me/crypto_parttime_jobs",
        "t.me/cyberabad_hiring_now"
    ]
    ...
    for c in range(1, num_chains + 1):
        ...
        target_atm = random.choice(atms) if atms else {
            "atm_id": "ATM_SBI_JAM_101",
            "bank_name": "State Bank of India",
            "lat": 24.2185,
            "lon": 86.6492,
            "h3_res9": "89618c0e667ffff"
        }
    ...
    acc_file = out_dir / "accounts.json"
    chain_file = out_dir / "chains.json"
