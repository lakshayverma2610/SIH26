...
try:
    ...
    print("⚠️ 'kafka-python' not installed. Exiting. (pip install kafka-python)")
    sys.exit(1)
...
from data_simulation.india_geo_catalog import get_random_indian_land_location, PAN_INDIA_DISTRICTS

def generate_random_location():
    """Generates coordinates strictly located on Indian landmass across all 36 States & UTs (0% Ocean)."""
    lat, lon, city, state = get_random_indian_land_location()
    return lat, lon
...
def continuous_background_worker(accounts, chains):
    ...
    while True:
        if STREAMING_ACTIVE and CURRENT_TPS > 0:
            ...
            if producer and tx_counter % 50 == 0:
                producer.flush()
...
def trigger_attack_scenario(chain, silent=False):
    ...
    terminal = chain.get("layer_3_terminal", {})
    if terminal and "lat" in terminal and "lon" in terminal and terminal["lat"] and terminal["lon"]:
        # Tightly converge attack within 500m to 1km radius of target ATM
        atk_lat = round(float(terminal["lat"]) + random.uniform(-0.005, 0.005), 5)
        atk_lon = round(float(terminal["lon"]) + random.uniform(-0.005, 0.005), 5)
    else:
        atk_lat, atk_lon = generate_random_location()

    terminal_name = terminal.get("bank_name", "Public Sector Bank ATM")
    ...
    # Step 1: Ingest 1930 NCRP Complaint (via Kafka in production, currently omitted for brevity)
...
def attack_submenu(chains):
    while True:
        ...
        print("  [1] Generic RAT Fraud")
        print("  [2] Generic Biometric Attack")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--auto", action="store_true", help="Start continuous stream automatically")
    parser.add_argument("--tps", type=int, default=10, help="Transactions per second")
    parser.add_argument("--attacks", type=int, default=5, help="Number of cybercrime attacks to inject in auto mode")
    parser.add_argument("--burst", type=int, default=0, help="Inject single attack burst and exit")
    ...
    if args.burst > 0:
        print(f"Injecting {args.burst} attack scenarios directly into Kafka...")
        for _ in range(args.burst):
            if chs:
                trigger_attack_scenario(random.choice(chs))
                time.sleep(0.5)
        print("✅ Finished burst injection.")
        sys.exit(0)

    if args.auto:
        CURRENT_TPS = args.tps
        STREAMING_ACTIVE = True
        print(f"🚀 Auto-streaming started at {CURRENT_TPS} tx/sec with continuous cybercrime attack injection...")
        bg_thread = threading.Thread(target=continuous_background_worker, args=(accs, chs), daemon=True)
        bg_thread.start()
        # Inject initial attacks
        for i in range(args.attacks):
            if chs:
                print(f"⚡ Injecting attack scenario {i+1}/{args.attacks}...")
                trigger_attack_scenario(random.choice(chs))
                time.sleep(1.0)
        print("Continuous background streaming active. Press Ctrl+C to stop.")
        try:
            while True:
                time.sleep(1)
