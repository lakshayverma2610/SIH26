...
class GeospatialPredictor:
    ...
    high-probability physical ATM cash-out targets via Redis GEO or in-memory models.
    """

    def __init__(self, atms_file_path: Optional[str] = None, redis_url: Optional[str] = None):
        ...
        # Connect to Redis for enterprise sub-millisecond Geospatial queries
    ...
    def find_nearest_atms(self, lat: float, lon: float, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        using Redis Geospatial search or fallback haversine matrix.
        """
        # 1. Enterprise Path: Query Redis GEO index
            ...
            try:
                ...
                if results:
                    ...
                    for item in results:
                        ...
                        atm_meta = self.redis_client.hgetall(f"atm:{atm_id}") or {}
                        prob = max(0.1, min(0.99, math.exp(-dist_km / 0.8)))
                        ranked_redis_atms.append({
                            "atm_id": atm_id,
                            "bank_name": atm_meta.get("bank_name", "Public Sector Bank ATM"),
                            "lat": coords[1],
                            "lon": coords[0],
                            "address": atm_meta.get("address", "Commercial Corridor"),
                            "distance_meters": round(dist_km * 1000.0, 1),
                            "withdrawal_probability": round(prob, 2),
                            "h3_res9": self.h3_indexer.latlng_to_h3(coords[1], coords[0], resolution=self.config.DEFAULT_H3_RES_KIOSK)
                        })
                    return ranked_redis_atms
            except Exception:
                pass

        # 2. Fallback Path: In-memory haversine scan
        if not self.atms:
            return []
